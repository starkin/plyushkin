...
describe Plyushkin::SimpleValue do
  describe '#new' do
    it 'should allow storing an integer' do
      val = Plyushkin::SimpleValue.new(:value => 5)
      val.value.should == 5
    end

    it 'should allow storing a string' do
      val = Plyushkin::SimpleValue.new(:value => 'test')
    ...
    it 'should use provided date if specified' do
      ...
      val  = Plyushkin::SimpleValue.new(:value => 5, :date => date)
